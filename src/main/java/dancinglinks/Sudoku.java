...
public class Sudoku {
    ...
    public static class ConstraintsGenerator {
        ...
        public ConstraintsGenerator(final int size, final Collection<Cell> existingValues, List<String> alphabeth) {
            ...
            upToN().forEach(
                    rowIndex -> upToN().forEach(
                            columnIndex -> upToN().forEach(
                                    number -> allPossibleCells.add(new Cell(rowIndex, columnIndex, alphabeth.get(number - 1))))));
        }
